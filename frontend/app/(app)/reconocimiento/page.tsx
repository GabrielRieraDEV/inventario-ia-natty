...
type Sesion = { estado: string; resultado: { producto: Producto; tiempo_respuesta_s: number } | null };
...
function Contenido() {
  ...
  async function confirmar() {
    ...
    setError(null);
    try {
      await api.post(`/api/sesiones/${token}/confirmar`, { cantidad, nombre });
      setOk(true);
    ...
    <div className="max-w-container-max mx-auto w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-xl items-start">
        ...
        <div className="lg:col-span-5 relative rounded-xl overflow-hidden border border-outline-variant bg-gradient-to-br from-surface-container to-surface-container-high shadow-sm h-[360px] lg:h-[520px] flex items-center justify-center">
          ...
          <div className="absolute inset-0 border-2 border-primary border-dashed opacity-40 m-lg rounded-lg" />
        ...
        <div className="lg:col-span-7 flex flex-col gap-lg">
          ...
          <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-lg shadow-sm">
            ...
            <div className="flex items-center justify-end gap-md pt-lg">
              <button onClick={() => router.push("/qr")} className="bg-surface-container-lowest border border-primary text-primary hover:bg-surface-container-high font-label-md text-label-md px-xl py-sm rounded-md transition-colors shadow-sm">
                Escanear otro
              </button>
              <button onClick={confirmar} disabled={!prod || guardando || ok}
                ...
                <Icon name="check" className="text-[18px]" /> {guardando ? "Registrando…" : "Confirmar registro"}
              </button>
            </div>
          </div>
        </div>
